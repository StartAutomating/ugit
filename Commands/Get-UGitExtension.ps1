    ...
    begin {
        ...
        function ConvertToExtension($toExtension) {
            ...
            $extCmd =
                ...
                elseif ($in -is [IO.FileInfo]) {
                    if ($in.LastWriteTime -gt $script:UGitExtensionsFileTimes[$in.Fullname]) {
                        $script:UGitExtensionsFileTimes[$in.Fullname] = $in.LastWriteTime
                        $script:UGitExtensionsFromFiles[$in.Fullname] = 
                            $ExecutionContext.SessionState.InvokeCommand.GetCommand($in.fullname, 'ExternalScript,Application')
                        $script:UGitExtensionsFromFiles[$in.Fullname]
                    } elseif ($script:UGitExtensionsFromFiles[$in.Fullname])  {
                        return $script:UGitExtensionsFromFiles[$in.Fullname]
                    }                    
                }
                else {
            ...
            if (-not $UGitExtensionTypeName) {
                $UGitExtensionTypeName = 'Extension'
            }
            if ($extCmd.pstypenames -notcontains $UGitExtensionTypeName) {            
                $extCmd.pstypenames.insert(0,$UGitExtensionTypeName)
            }
        ...
        if (-not $script:UGitExtensions)
        {
            ...
            $script:UGitExtensions =
                ...
                #region Find UGitExtension in Loaded Commands                
